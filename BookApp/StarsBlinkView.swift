...
struct StarsBlinkView: View {
            ...
            ZStack {
                ...
                ForEach(stars) { star in
                    Star()
                        ...
                        .animation(Animation.easeInOut(duration: 1.0).repeatForever(autoreverses: true), value: star.position)
                }
    ...
    private func startAnimatingStars(in size: CGSize) {
        let timer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { timer in
            ...
            let newStar = StarProperties(position: CGPoint(x: randomX, y: randomY), scale: randomSize, hue: randomHue)
